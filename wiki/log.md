...
## [2026-07-06] resolve | Ijen health-screening wording flipped conditional → mandatory (policy-bundle compiler inputs)

Owner adjudication 2026-07-06 (recorded in jvto-web `docs/CANONICAL_FACTS.md` via commit `d3b4d11`, PR #83) supersedes the 2026-07-05 conditional decision: a health certificate is mandatory for every Ijen guest, with BBKSDA SE.1658/KSA.9/2024 cited as supporting authority/evidence, not a conditional trigger. Triggered by a Codex review finding on jvto-web PR #84 (`claude/policy-bundle-extraction`) — the synced `consumer-bundles.json` still carried the old conditional wording/notes for the `whatsapp` → `ijen-health-screening` policy.

Pages updated (the 4 canonical/supporting sources for the `Ijen Health Screening` policy domain per `wiki/ops/policy-source-ownership.md`, plus that ownership table itself): `wiki/ops/policy-source-ownership.md` (ownership-table notes), `wiki/website/faq-master.md` (Q6 flag + answer text, schema-notes line), `wiki/credentials/medical-screening.md` (frontmatter flag, voice invariant, approved-language table, regulatory-basis note, AEO snippet), `wiki/destinations/kawah-ijen.md` (frontmatter flag, wording-mode table row, approved-language section, content-angle bullet), `wiki/people/dr-ahmad-irwandanu.md` (frontmatter flag, section header + voice invariant + approved/forbidden lists, protocol intro). `wiki/sources/ssot-v6.md` kept verbatim as historical record with a superseded-2026-07-06 annotation on anti-drift rule 6, since it summarizes an already-ingested raw source.

Recompiled `output/website/policy-bundle/*` via `python3 scripts/compile_policy_bundle.py --write --verbose` — 0 findings, `manifest.clean: true`. Only the `ijen-health-screening` policy's `notes` + Q6 evidence `text` changed in `policy-bundle.json`/`consumer-bundles.json`; `_manifest.json` timestamp + faq-master source hash updated. No other policy domain touched.

Scope note: this pass covers only the policy-bundle compiler's source inputs (the files the compiler actually reads), not the wider wiki (aeo-claims.md, brand-voice.md, copy-bank.md, website-context-master.md, output/website/pages/**, schema receipts, etc.) — those still show old "conditional"/"can require" phrasing in places and are a separate full-repo wording sweep, consistent with jvto-web's own "full-repo wording sweep deferred to Global Validator wedge" note.

## [2026-07-06] ops | Audit-cleanup pass — CI gate hardening (Phase 3a/b)

Repo-wide branch audit (16 non-master branches, all 25 historical PRs cross-referenced against `master` by content diff, not ancestry — this repo's `merged` PR flag is unreliable since content typically lands separately from the PR merge button). 14 branches confirmed 100% superseded (tag/branch-backed as `backup/*`, pending owner confirmation to delete remote refs). 1 branch (`claude/epic-brahmagupta-SzDui`) had real unlanded work — landed via #26: `plugins/llm-wiki/commands/blog-preview.md` + `blog-publish.md` (the skill already had Phase 2/3 defined, just missing command entry-points) + 3 `.claude/hookify.*.local.md` guardrails.

Audit also surfaced two CI gaps: `compile_policy_bundle.py` and `scripts/verify_claims.py` existed but were never wired into `.github/workflows/ci.yml`'s `validate` job, and `pytest` (declared in `requirements-dev.txt`, full `tests/` tree) was never run in CI. Before wiring `verify_claims.py --all` in as a required gate, ran it locally first — found it currently fails (7 violations) on `master`:

- **`MEDICAL-UNIVERSAL` rule removed** from `scripts/claim_boundaries.yml` — it banned "no valid QR = no access" / "mandatory and non-negotiable" as a universal-JVTO-rule over-claim, but the 2026-07-06 owner adjudication (same one that flipped `docs/CANONICAL_FACTS.md` in jvto-web) made mandatory wording the *correct* framing, superseding the 2026-07-05 conditional decision. The rule was banning exactly the wording now required — same fix jvto-web made in commit `d3b4d11` to `validate-content-drift.mjs`. Removed the corresponding test case in `tests/test_verify_claims.py`.
- **2 genuine stale-review-count fixes**: `output/website/pages/verify-jvto/hub.md` and `output/website/pages/why-jvto/reviews.md` still showed Google Maps at the retired 92/4.90 value; updated to canonical 123/4.9 (reviews.md verified-date also corrected to 2026-05-26, matching the API check date recorded elsewhere).
- **2 false-positive fixes**: `output/website/schema/homepage-organization-schema.receipt.md` and `output/website/trust-bundle/extended-bundle-receipt.md` are drift-resolution receipts that mention the old "92" value only to document that it was already fixed — reworded both to include "superseded" so the linter's existing `context_markers` mechanism (designed for exactly this) correctly treats them as historical notes, not active claims, instead of adding a one-off suppression.

After these fixes: `pytest` 111/111 passed, `verify_claims.py --all` clean (157 files scanned), `compile_policy_bundle.py --dry-run --strict --verbose` 0 findings. Added all three (`compile_policy_bundle.py --strict`, `verify_claims.py --all --quiet`, `pytest -q`) as required steps in `ci.yml`, matching the existing step style.

Next in this pass: extract shared compiler primitives (`scripts/compiler/`, `scripts/package_compiler/`, `scripts/policy_compiler/` each hand-roll the same manifest/CLI scaffold) into `scripts/compiler_common/` — must be output-byte-identical, verified by diff before landing.

## [2026-07-07] compile | trust-bundle v0.1.0 — 9 claims, 0 errors, 0 warnings

## [2026-07-07] resolve | C4 mandatory-wording propagation sweep + MEDICAL-CONDITIONAL guard
Completed the 2026-07-06 mandatory-screening adjudication propagation that the compiled/output layer had missed. Inputs reconciled: aeo-claims.md (C4 narrative + voice invariant + routing/rules), brand-voice.md + copy-bank.md (approved/forbidden lists flipped), website-context-master.md §3.1 (Conditional Rule -> Mandatory Rule + forbidden table), faq-master.md Q14. Recompiled: trust bundle (compile_trust.py, F1-F8 pass — claims.json C4 narrative now mandatory) + policy bundle (Q14). Hand-maintained outputs fixed: policies.json health_screening (wording_rule MANDATORY, approved/forbidden phrase sets swapped), homepage, tours.md, 13 tour pages, 6 travel-guide pages, 2 landing pages, destinations/ijen-crater, markets/taiwan, blog.md + ijen-medical-checkup blog post, aeo/ijen + gas-mask, faq/ijen, llms-full.txt, social batch. New linter rule MEDICAL-CONDITIONAL in claim_boundaries.yml (inverse of the removed MEDICAL-UNIVERSAL) caught 53 stale instances; verify_claims.py --all now clean (157 files). Trigger: knowledge-catalog-jvto-bootstrap PR #27 propagation recommendation.
