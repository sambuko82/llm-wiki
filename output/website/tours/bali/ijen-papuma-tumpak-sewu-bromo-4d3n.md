...
page: /tours/from-bali/ijen-papuma-tumpak-sewu-bromo-4d3n
...
# Tour Page Data Structure: /tours/from-bali/ijen-papuma-tumpak-sewu-bromo-4d3n

> Tour data fetched from DB at runtime. This file defines display schema only.
> Tour name (DB): 4 Day Ijen, Papuma Beach, Tumpak Sewu & Bromo Expedition from Bali to Surabaya
...
| `origin_city` | DB → `tours.origin` | Display as badge: "From Bali" |
| `terminus_city` | DB → `tours.terminus` | Display as badge: "Ends in Surabaya" |
...
| Label | Source | Notes |
|---|---|---|
| Origin | DB → `tours.origin` | "Bali" |
| Ends In | DB → `tours.terminus` | "Surabaya" — display when terminus ≠ origin |
...
| Ferry Crossing | Static | "Bali–Java ferry (Gilimanuk–Ketapang) included" |
...
| Ferry crossing | Static (Policy) | "Bali–Java ferry tickets (Gilimanuk–Ketapang) included" |
...
| Each tier: `pax_label` | DB | e.g., "1 pax (solo)", "2 pax", "3 pax", "4–5 pax" |
| Each tier: `price_idr` | DB | IDR per person, format: IDR X,XXX,XXX |
| Solo surcharge note | Static | "Solo traveller rate includes single-occupancy supplement" |
...
- **Terminus badge / "Ends in Surabaya" row**: SHOW — `terminus: surabaya`
- **Terminus advisory**: SHOW — "Confirm onward travel from Surabaya before booking."
- **Bromo Jeep line in inclusions**: SHOW — Bromo is in this package
- **Ferry crossing line in inclusions**: SHOW — Bali origin, ferry Gilimanuk–Ketapang included
- **Solo pax tier**: SHOW in pricing table when `pricing_tiers` includes a `1 pax` entry

---

## Notes

- `terminus: surabaya` — this tour starts in Bali and ends in Surabaya.
- Display advisory: "Confirm onward travel from Surabaya before booking."
- Ferry crossing (Gilimanuk–Ketapang) is a standard Bali-origin inclusion.
- Package includes solo (1 pax) tier per pricing benchmarks (IDR 9,050,000/person).
