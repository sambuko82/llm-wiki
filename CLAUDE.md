...
**Last completed:** Homepage Google Maps review-count fix (commit `63ab1eb`) — stale "92 on Google Maps" → canonical "123" in `output/website/pages/homepage.md` (L183 heading + L187 intro). Verified vs `trust-signals.md` §Live Review Platforms (Google 4.9/5, 123, 2026-05-26 API); SSOT flagged 92 as stale. Sprint named only L183; grep found stale 92 in both spots — both fixed. TP 51 + TA 21 + all ratings already matched SSOT, untouched. Logged in `wiki/log.md`. GAP-05 now resolved in-repo (jvto-web live deploy still mirrors separately).
